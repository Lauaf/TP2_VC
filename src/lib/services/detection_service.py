...
class DetectionService:
    ...
    def detect_dogs(self, image: np.ndarray) -> list[tuple[tuple[int, int, int, int], float]]:
        ...
        if image is None or image.size == 0:
            raise ValueError("Image is empty; cannot detect dogs.")
        ...
        for result in results:
            ...
            for box, score, class_id in zip(xyxy, scores, classes, strict=False):
                if class_id != self.dog_class_id or float(score) < self.conf_threshold:
                    continue
    ...
    def classify_detected_dog(self, crop: np.ndarray) -> tuple[str, float]:
        ...
        if crop is None or crop.size == 0:
            return "unknown", 0.0
        return self.classifier._predict_from_bgr(crop)
